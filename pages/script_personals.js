// Save bid as a note
document.getElementById('personalsForm').addEventListener('submit', function(event) {
    ...
    const bidText = `${bidPrefix} Need to ${action} approx ${perscount} cyd(s) of personal property consisting of ${consist}. ${movef ?  `Personals will be moved from ${movef} to ${movet}` : ''} ${storesize ?  `Will require renting a ${storesize} storage for 30 days before disposing the personals to let the owner retrieve the belongings.` : ''} ${additionalInfo}. ${conj ? `***Must be approved in conjunction to ${conj} bid.` : ''} \n${store ? `Facility Name: ${store}` : ''}\n${address ? `Facility Location: ${address}` : ''}\n${contact ? `Facility Contact Number: ${contact}` : ''} \nPrice: $${formattedPrice}`;
