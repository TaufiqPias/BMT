// Save bid as a note
document.getElementById('fenceForm').addEventListener('submit', function(event) {
    event.preventDefault();

    // Get form values
    const bidPrefix = document.getElementById('bidPrefix').value;
    const fenceaction = document.getElementById('fenceaction').value;
    ...
    const toprailcaps = document.getElementById('toprailcaps').value;
    ...
    const bidText = `${bidPrefix} Need to ${fenceaction} ${length} LF of ${fencetype} fence from/at the ${location} side of the property. Reason: ${reason}. ${debris ? `Will generate approx ${debris} cyds of debris.` : ''} Will need to ${fenceaction} ${panelcount} pieces of ${panelsize} panels and ${postcount} posts. ${toprailcaps ? `${toprailcaps}to ${fenceaction} ${length} LF toprail and ${postcount} pieces of loop caps/ post caps.` : ''} ${gate ? `Will need to ${fenceaction} ${gate}` : ''} Will require ${equipments}. ${conj ? `***Must be approved in conjunction to ${conj} bid.` : ''} ${additionalInfo}\nPrice: $${formattedPrice}`;
