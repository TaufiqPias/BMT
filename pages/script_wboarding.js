// Save bid as a note
document.getElementById('WBForm').addEventListener('submit', function(event) {
    ...
    const wcount = document.getElementById('wcount').value;
    ...
    const windowInputs = windowFields.querySelectorAll('input[type="text"]');
    let windowSizes = '';

    windowInputs.forEach((input, index) => {
        if (input.value) {
            windowSizes += `Window ${index + 1} size: ${input.value}\n`;
        }
    });

    // Generate the bid text
    const bidText = `${bidPrefix} Need to ${action} ${wcount} window(s) ${material}. Structure: ${structure}. ${debris ? `Will generate approx ${debris} cyds of debris - includes hauling away.` : ''}\nWindow Size:\n${windowSizes}${conj ? `***Must be approved in conjunction to ${conj} bid.***` : ''} ${additionalInfo}\nPrice: $${formattedPrice}`;
