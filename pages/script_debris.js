...
document.getElementById('debForm').addEventListener('submit', function(event) {
    ...
    const debcount = document.getElementById('debcount').value;
    ...
    const bidText = `${bidPrefix} Need to ${action} approx ${debcount} cyd(s) of ${bidtype} consisting of ${consist}. ${dumpster ? `Will require renting a ${dumpster} to haul away the debris.` : ''} If approved, will provide empty load, half load and full load measurement photos including proper length x width x height. ${conj ? `***Must be approved in conjunction to ${conj} bid.***` : ''} ${additionalInfo}\nPrice: $${formattedPrice}`;
