// Save bid as a note
document.getElementById('shrubsForm').addEventListener('submit', function(event) {
    event.preventDefault();

    // Get form values
    const bidPrefix = document.getElementById('bidPrefix').value;
    const action = document.getElementById('action').value;
    const shrubsCount = document.getElementById('shrubsCount').value;
    const shrubsize = document.getElementById('shrubsize').value;
    ...
    const bidText = `${bidPrefix} Need to ${action} ${shrubsCount} LF of ${shrubsize} shrubs at/around the ${location} of the property due to ${reason}. Will generate approx ${debris} cyds of debris. Will require ${equipments}. ${conj ? `***Must be approved in conjunction to ${conj} bid.` : ''} ${additionalInfo}\nPrice: $${formattedPrice}`;
